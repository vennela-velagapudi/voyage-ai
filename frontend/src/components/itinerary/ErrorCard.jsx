...
export default function ErrorCard({ error, onRetry, onEdit }) {
  ...
  let accentColor = 'from-rose-500/20 to-amber-500/10 border-rose-500/30 text-rose-400';
  ...
  if (error) {
    ...
    if (rawMsg.includes('timeout') || error.code === 'ECONNABORTED') {
      ...
      Icon = WifiOff;
      accentColor = 'from-rose-600/20 to-slate-900 border-rose-500/40 text-rose-400';
    } else if (status === 400 && backendMsg) {
      title = 'Trip Parameters Need Attention';
      message = backendMsg;
      Icon = AlertCircle;
      accentColor = 'from-amber-500/20 to-yellow-500/10 border-amber-500/40 text-amber-300';
    } else if (backendMsg) {
      title = 'AI Generation Alert';
      message = backendMsg;
      Icon = AlertTriangle;
      accentColor =
        'from-rose-500/20 via-indigo-900/40 to-slate-900 border-rose-500/30 text-rose-400';
    } else if (rawMsg) {
      message = rawMsg;
    }
  }
  ...
  return (
    <motion.div
      ...
      className={`w-full max-w-2xl mx-auto my-8 p-8 sm:p-10 rounded-3xl bg-gradient-to-br ${accentColor} bg-slate-950/90 border backdrop-blur-xl shadow-2xl relative overflow-hidden text-center sm:text-left`}
    >
      {/* Subtle top light */}
      ...
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
        <div className="w-16 h-16 rounded-2xl bg-rose-500/15 border border-rose-500/30 flex items-center justify-center flex-shrink-0 text-rose-400 shadow-[0_0_20px_rgba(244,63,94,0.2)]">
          <Icon className="h-8 w-8 animate-pulse" />
        </div>

        <div className="flex-1">
          <h3 className="text-xl sm:text-2xl font-display font-extrabold text-white tracking-tight mb-2.5">
            {title}
          </h3>
          <p className="text-slate-300 text-sm sm:text-base leading-relaxed mb-8 font-normal bg-slate-900/60 p-4 rounded-2xl border border-slate-800/80">
            {message}
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3.5">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              onClick={onEdit}
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-6 py-3.5 rounded-xl bg-slate-800/90 hover:bg-slate-700/90 text-slate-200 border border-slate-700/60 text-sm font-semibold transition-all cursor-pointer shadow-sm"
            >
