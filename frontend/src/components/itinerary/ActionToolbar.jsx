...
export default function ActionToolbar({
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-4xl mx-auto my-12 p-4 sm:p-6 rounded-2xl glass-effect border border-border-theme bg-surface-card/85 backdrop-blur-xl shadow-xl flex flex-col sm:flex-row items-center justify-between gap-4"
    >
      <div className="flex items-center gap-2 text-sm text-text-body font-medium text-center sm:text-left">
        <Sparkles className="h-4 w-4 text-indigo-600 dark:text-indigo-400 flex-shrink-0 animate-pulse" />
      ...
      <div className="flex flex-col sm:flex-row flex-wrap items-center justify-center sm:justify-end gap-3 w-full sm:w-auto">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          onClick={onEditForm}
          className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-surface-inner hover:bg-surface-hover text-text-body text-xs sm:text-sm font-semibold border border-border-theme transition-all shadow-sm cursor-pointer"
        >
