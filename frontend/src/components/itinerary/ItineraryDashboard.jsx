...
export default function ItineraryDashboard({
  ...
  // Maintain persistent live React state for interactive edits (Milestone 5 & 6)
  ...
  // Centralized helper to persist any interactive reordering or edit directly into React state
  const persistItineraryState = (newTripData) => {
    setTripData(newTripData);
    if (onUpdateItinerary) {
      onUpdateItinerary(newTripData);
    }
  };
  ...
  const handleDeleteActivity = (dayIndex, activityIndex) => {
    ...
    persistItineraryState({ ...tripData, dailyItinerary: updatedDays });
  };
  ...
  const handleReorderActivities = (dayIndex, fromIndex, toIndex) => {
    ...
    updatedDays[dayIndex] = targetDay;
    // Persist reordered activities directly into current React itinerary state before any regeneration request
    persistItineraryState({ ...tripData, dailyItinerary: updatedDays });
  };
  ...
  const handleMoveDay = (fromIndex, toIndex) => {
    ...
    // Instantly update UI and React state ONLY - no API calls, no page reload, no automatic regeneration
    persistItineraryState({ ...tripData, dailyItinerary: reorderedDays });
  };
  ...
  // 4. Regenerate a single day schedule using latest edited/reordered itinerary state as baseline
  const handleRegenerateDay = async (dayIndex, passedDay) => {
    // ALWAYS pull the exact latest user-edited day and full itinerary from current React state
    const latestDay = tripData.dailyItinerary[dayIndex] || passedDay;
    setRegeneratingDayIndex(dayIndex);
    try {
      const response = await ApiService.regenerateDay({
        dayNumber: latestDay.dayNumber || dayIndex + 1,
        theme: latestDay.theme || 'Explore Highlights',
        destination: tripData.destination || userParams.destination || '',
        budget: tripData.budget || userParams.budget || 'moderate',
        travelStyle: tripData.travelStyle || userParams.travelStyle || 'travel',
        interests: tripData.interests || userParams.interests || 'sightseeing',
        notes: userParams.notes || '',
        existingDay: latestDay,
        existingItinerary: tripData,
      });
      const newDay = response?.day || response?.data?.day;
      if (newDay && newDay.timeline) {
        // Only update state and refresh UI after the new regenerated itinerary is returned from backend
        const updatedDays = [...tripData.dailyItinerary];
    ...
    >
      ...
      <div className="w-full max-w-4xl mx-auto mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-2xl bg-gradient-to-r from-indigo-950/60 via-slate-900/90 to-slate-900 border border-indigo-500/30 shadow-lg">
        ...
        <button
          ...
          onClick={() => onRegenerate && onRegenerate(tripData)}
          className={`w-full sm:w-auto px-5 py-2.5 rounded-xl font-extrabold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-lg transition-all cursor-pointer flex-shrink-0 ${
            isRegenerating
              ? 'bg-indigo-600/40 text-indigo-200 border border-indigo-500/50 cursor-wait animate-pulse'
              : 'bg-gradient-to-r from-indigo-600 to-sky-600 hover:from-indigo-500 hover:to-sky-500 text-white shadow-indigo-600/30 hover:shadow-indigo-600/50 transform hover:-translate-y-0.5'
          }`}
        >
          <RefreshCw className={`h-4 w-4 ${isRegenerating ? 'animate-spin' : ''}`} />
          <span>{isRegenerating ? 'Generating New Plan...' : 'Rearrange & Generate New Plan'}</span>
        </button>
      </div>
