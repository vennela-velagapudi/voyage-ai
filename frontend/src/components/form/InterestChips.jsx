        ...
        {options.map((interest) => {
          ...
          return (
            ...
            >
              {ChipIcon && (
                <ChipIcon
                  className={`h-3.5 w-3.5 ${isSelected ? 'text-indigo-400' : 'text-slate-500'}`}
                />
              )}
              <span>{interest.label}</span>
              {isSelected ? (
                <Check className="h-3 w-3 text-indigo-400 ml-0.5" />
              ) : (
                <Plus className="h-3 w-3 text-slate-600 ml-0.5" />
              )}
            </motion.button>
          );
        })}
